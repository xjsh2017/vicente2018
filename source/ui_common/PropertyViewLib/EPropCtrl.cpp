...
/*************************************************************
 函 数 名: FindByItemData()
 功能概要: 根据ItemData查找对应的属性行对象
 返 回 值: 对应的属性行对象
 参    数: param1 ItemData值
		   Param2 
**************************************************************/
EProperty* EPropCtrl::FindByItemData( DWORD dData )
{
	return m_PropListCtrl.FindByItemData(dData);
}