...
void CDlgTagOutSet::FillData()
{
	...
	for(int i = 1; i <= nRowCount; i ++)
	{
		...
		m_List.SetItemText(nIndex, 2, usrGroupID); // 执行用户

		m_List.SetItemText(nIndex, 3, usrID); // 执行用户
		
		m_List.SetItemText(nIndex, 4, QByteArray::number(nEnable).constData()); // 是否启用
...
BOOL CDlgTagOutSet::CheckFlow()
{
	...
	if (pApp->m_User.m_strGROUP_ID == CString(pUserStore->GetUserGroupIDName(XJ_USERGROUP_RUNNER).constData())
		...
		if (baLoginUserOwner == baOperOwner){
			msg << "[101 检查项]：运行组和操作组指定的用户不能是同一所有者" << "\n"
				...
				<< "[检查结果]：未通过" << "\n\n"
				<< "[原因说明]：";
			str.Format("挂牌用户(%s)和操作员(%s)(第%d行)为同一人(%s)所有"
	...
	if (!baRunner.isEmpty() && !baOper.isEmpty()){
		if (baRunnerOwner == baOperOwner){
			msg << "[101 检查项]：运行组和操作组指定的用户不能是同一所有者" << "\n"
				...
				<< "[检查结果]：未通过" << "\n\n"
				<< "[原因说明]：";
			str.Format("运行员(%s)(第%d行)和操作员(%s)(第%d行)为同一人(%s)所有"
	...
	if (!baMonitorOwner.isEmpty() && !baOper.isEmpty()){
		if (baMonitorOwner == baOperOwner){
			msg << "[102 检查项]：操作组和监护组指定的用户不能是同一所有者" << "\n"
				<< "-------------------------\n"
				<< "[检查结果]：未通过" << "\n\n"
				<< "[原因说明]：";
			str.Format("操作员(%s)(第%d行)和监护员(%s)(第%d行)为同一人(%s)所有"
