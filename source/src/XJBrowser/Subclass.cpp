...
//
//##ModelId=49B87BB603A9
class CSubclassWndMap : private CMapPtrToPtr {
public:
	//##ModelId=49B87BB603AB
	CSubclassWndMap();
	//##ModelId=49B87BB603B9
	~CSubclassWndMap();
	//##ModelId=49B87BB603BA
	static CSubclassWndMap& GetHookMap();
	//##ModelId=49B87BB603BC
	void Add(HWND hwnd, CSubclassWnd* pSubclassWnd);
	//##ModelId=49B87BB603BF
	void Remove(CSubclassWnd* pUnHook);
	//##ModelId=49B87BB603C1
	void RemoveAll(HWND hwnd);
	//##ModelId=49B87BB603C8
...
//
LRESULT CALLBACK
SubHookWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
#ifdef _USRDLL
...
void CSubclassWndMap::Add(HWND hwnd, CSubclassWnd* pSubclassWnd)
{
	...
	if (pSubclassWnd->m_pNext==NULL) {
		...
		pSubclassWnd->m_pOldWndProc = 
			(WNDPROC)SetWindowLong(hwnd, GWL_WNDPROC, (DWORD)SubHookWndProc);
