...
{ 
   ...
   //ÒÔÏÂadd by yinzhehong 20060816 ½â¾öÊý×ÖÅÅÐòÎÊÌâ
	int nKoef = bAscending ? 1 : -1;
	if(IsNumber(sText))
	{
		return NumberCompare(sText,sTextToCompare) * nKoef > 0;
	}
	else
	{
...
//ÊÇ·ñÊý×Ö
BOOL COXTreeItem::IsNumber(CString &str)
{
	if(str.IsEmpty())
	{
		return FALSE;
...
//¶ÔÊý×Ö½øÐÐ±È½Ï
int COXTreeItem::NumberCompare(CString &str, CString &strNext)
{
	double f1 = atof( str.GetBuffer(MAX_PATH) );
	double f2 = atof( strNext.GetBuffer(MAX_PATH) );
