	...
	UINT nIndex, BOOL bSysMenu)
{
	if (m_bAutoAccel)
	{
		...
		HACCEL hAccel = m_pFrame->GetTopLevelFrame()->GetDefaultAccelerator();
		
		if (hAccel != m_hAccel)
			LoadAccel(hAccel);

		//1999.6.14 comment out by Li Xiang
	...
	UINT nIndex, BOOL bSysMenu, BOOL bShowButtons)
{
	...
	for (UINT i = 0; i < nItem; i++) {	
		...
		//delete the bSysMenu, made by lihang
		if (info.wID >= 0xF000) {
