...
BOOL CDlgValidateID::OnInitDialog() 
{
	...
	if (m_nPersonType ==0)
	{
		SetWindowText( StringFromID(IDS_VALIDATE_RUNNER));
	}
	else if (m_nPersonType ==1)
	{
	...
	try
	{
		...
		if(pMemset != NULL && nResult == 1)
		{
			...
			else
			{
				...
				if (m_strUserGroup == StringFromID(IDS_USERGROUP_SUPER)){

				}else if (0 == m_nPersonType){
					if (m_strUserGroup != StringFromID(IDS_USERGROUP_RUNNER) 
						&& m_strUserGroup != StringFromID(IDS_USERGROUP_SUPER)){
						CString str;
						str.Format("用户权限不足，条件为：[%s] 或者 [%s]"
