...
m_ptLastClick(0,0),m_bInit(FALSE)//m_wndCal()
{
...
BEGIN_MESSAGE_MAP(COXTreeCtrl, CListCtrl)
	...
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
	ON_NOTIFY_REFLECT_EX(LVN_KEYDOWN, OnKeydown)
	ON_WM_PARENTNOTIFY()
	ON_WM_DESTROY()
	ON_WM_TIMER()
	ON_WM_STYLECHANGED()
				...
				for(nCol=0; nCol<nColCnt; nCol++)
				{
					...
					if(dc.RectVisible(rcItem))
					{
						...
						clrOldForground=dc.SetTextColor(bShowItemSel && 
							...
							::GetSysColor(COLOR_3DSHADOW) : (xti->HasColor(nCol) ?
							xti->GetItemColor(nCol) : GetTextColor())));
						clrOldBackground=dc.SetBkColor(
							bShowItemSel && (nCol==0 || bRowSel) ? 
						...
						DrawItemText(&dc,sTxt,rcItem,lvc.fmt,
							...
							::GetSysColor(COLOR_BTNFACE)) : 
							(!IsWindowEnabled() ? ::GetSysColor(COLOR_BTNFACE) : 
							GetTextBkColor()),nEllipsisFormat);
						dc.SetTextColor(clrOldForground);
						dc.SetBkColor(clrOldBackground);
...
void COXTreeCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(idx!=-1)
	{
		if((nFl & TVHT_ONITEMBUTTON)!=0 && ItemHasChildren(hti))
		{
...
int COXTreeCtrl::SetItemAtPos(int pos,COXTreeItem *pItem)
{
	...
	if(idx == -1)
		return -1;
	CListCtrl::SetItemData(idx,(DWORD)pItem);
	...
	//--- Effect :
{
	...
	if((pItem->pxParent!=NULL && !pItem->pxParent->IsExpanded()) || pItem->IsHidden())
		return TRUE;
...
void COXTreeCtrl::DrawLines(CDC *pDC, LPRECT rLines, COXTreeItem *pItem, int nLev)
{
	...
	CPen penBtn(PS_SOLID, 1, RGB(160, 0, 0));
...
BOOL COXTreeCtrl::GetItem(TV_ITEM *pTVI) const
{
	...
	if(pTVI->mask & TVIF_TEXT)
	{
		...
		{
			_tcsncpy(pTVI->pszText,xti->m_tvi.pszText,pTVI->cchTextMax);
		}
	}
...
BOOL COXTreeCtrl::SetSubItem(HTREEITEM hItem, int nColumn, UINT uFlags,
							 LPCTSTR lpszText, int nImage, CFont *pFont, COLORREF clr)
{
	COXTreeItem *xti=GetXItem(hItem);
	if(!xti)
		return FALSE;
	if(nColumn <= 0)
		return FALSE;
	BOOL bRet=xti->SetSubItem(nColumn,uFlags,lpszText,nImage,pFont,clr);
	int idx=GetItemIndexInternal(xti);
	if(bRet && idx != -1)
	{
		CListCtrl::RedrawItems(idx,idx);
...
BOOL COXTreeCtrl::EnsureVisible(HTREEITEM hti)
{
	...
	if(!xti->IsVisible())
	{
		...
		{
			for(int nIndex=arrItemsToExpand.GetSize()-1; nIndex>=0; nIndex--)
...
BOOL COXTreeCtrl::OnKeydown(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	*pResult=0;
	...
	if(nCount>1)
	{
		...
		TCHAR chSymbol[2];
		chSymbol[0]=(TCHAR)pLVKeyDown->wVKey;
		chSymbol[1]=0;
		...
		m_nTimerCheckKeyboardInput=SetTimer(IDT_OXTREECTRL_CHECKFORKEYBOARDINPUT,
						 ...
						 UINT nID, DWORD dwExStyle/*=0*/)
{
	...
	DWORD dwAddStyle=::GetWindowLong(m_hWnd,GWL_STYLE);	
...
void COXTreeCtrl::OnContextMenu(CWnd* pWnd, CPoint point) 
{
