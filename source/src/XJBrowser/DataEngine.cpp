...
void CDataEngine::RefreshAllSecFromDB()
{
	CXJBrowserApp* pApp = (CXJBrowserApp*)AfxGetApp();
	//组建查询条件
	SQL_DATA sql;
	...
	{
		e->Delete();
		WriteLog("CDataEngine::RefreshAllSecFromDB, 查询失败");
		delete[] sError;
		delete pMemset;
		return;
	}
	if(pMemset != NULL && nResult == 1)
	{
		...
		if(nCount >= 0)
		{
			str.Format("CDataEngine::RefreshAllSecFromDB, 读取到%d条数据", nCount);
			WriteLog(str);
		}
		for(int i = 0; i < nCount; i++)
		{
			CString strPTID = pMemset->GetRealDataValue("pt_id"); //ID
			CSecObj* pObj = (CSecObj*)FindDevice(strPTID, TYPE_SEC);
			if (NULL != pObj){
				str = pMemset->GetRealDataValue("run_status"); //运行状态
				pObj->m_nRunStatu = atoi(str);
				str = pMemset->GetRealDataValue("commu_status"); //通讯状态
				pObj->m_nCommStatus = atoi(str);
				str = pMemset->GetRealDataValue("suspendstatus");
				pObj->m_nSuspendstatus = atoi(str); //挂牌状态
				str = pMemset->GetRealDataValue("suspendreason");
				pObj->m_nSuspendreason = atoi(str); //挂牌原因ID
					
				//读取挂牌信息
				pObj->m_sSuspendreason = pObj->GetMarkInfo(pObj->m_nSuspendreason);
				//读取CPU信息
				//	GetCPUInfo();
				//设置通讯状态
				pObj->SetCommStatus(pObj->m_nCommStatus);
				//	RefreshCommDurativeTime();
				//设置运行状态
				pObj->SetRunStatus(pObj->m_nRunStatu);
	...
	{
		CString str;
		str.Format("CDataEngine::RefreshAllSecFromDB,查询失败,原因为%s", sError);
		WriteLog(str);
	}
	delete[] sError;
	delete pMemset;
