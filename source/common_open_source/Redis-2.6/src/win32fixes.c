...
#ifdef OS_WINDOWS
#if _MSC_VER==1200
_SETFILEPOINTEREX SetFilePointerEx=NULL;
...
int replace_ftruncate(int fd, UNI_INT64 length) {
	...
	if (!SetFilePointerEx)
	{
		SetFilePointerEx=(_SETFILEPOINTEREX )GetProcAddress(GetModuleHandle("Kernel32.dll"),"SetFilePointerEx");
	}
#endif
#endif	
...
int win32_pthread_join(pthread_t *thread, void **value_ptr)  {
	...
	if (!OpenThread)
	{
		OpenThread=(_OPENTHREAD)GetProcAddress(GetModuleHandle("Kernel32.dll"),"OpenThread");
	}
#endif
#endif
...
double wstrtod(const char *nptr, char **eptr) {
    ...
    d = strtod(nptr, &leptr);//d = _strtod_l(nptr, &leptr, clocale);
