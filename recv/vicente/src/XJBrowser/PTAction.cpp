...
int CPTAction::InitListStyle()
{
	...
	{
		//载入配置失败
		//char* arColumn[PTACTION_COL_NUM]={"序号","类别", "故障类型", "装置启动时间", "信息接收时间", "相对时间","消息内容", "已确认"};
...
BOOL CPTAction::LoadListConfig()
{
	...
	CoInitialize(NULL); //初始化COM
	{
		...
		{  
			...
			WriteLog("PTAction::读取UIConfig失败,将使用默认列风格", XJ_LOG_LV3);
			bResult = FALSE;
		} 	
		//加载UIConfig文件 
		if(bResult)
		...
		if(bResult)
		{
			...
			{	
				//未找到
				WriteLog("未找到PTActionConfig", XJ_LOG_LV3);
				WriteLog("PTAction::读取UIConfig失败,将使用默认列风格", XJ_LOG_LV3);
				bResult = FALSE;
			}
			else
			{	//找到
				...
				{
					//找不到子节点
					WriteLog("未找到PTActionConfig下的配置", XJ_LOG_LV3);
					WriteLog("PTAction::读取UIConfig失败,将使用默认列风格", XJ_LOG_LV3);
					bResult = FALSE;
				}
				else
				{
					...
					{
						WriteLog("PTAction::列数量发生变化,不读取UIConfig,将使用默认列风格", XJ_LOG_LV3);
						bResult = FALSE;
...
BOOL CPTAction::SaveListConfig()
{
	...
	CoInitialize(NULL); //初始化COM
	{
		...
		{  
			WriteLog("无法创建DOMDocument对象，请检查是否安装了MS XML Parser 运行库!", XJ_LOG_LV3);
			WriteLog("PTAction::保存UIConfig失败", XJ_LOG_LV3);
			bResult = FALSE;
		} 	
		...
		if(bResult)
		{
			...
			else
			{
				...
				else //找到了PTActionConfig
				{
					//查找是否有子节点
					if(!pSelected ->hasChildNodes())
					{
						//找不到子节点
						WriteLog("未找到PTActionConfig下的配置, 新建所有子节点", XJ_LOG_LV3);
						//新建所有子节点
						for(int i = 0; i < m_arrColum.GetSize(); i++)
						{
							MSXML2::IXMLDOMElementPtr pNewChild = NULL;
					...
					{
						//找到有子节点, 删除所有子节点,再新建
						WriteLog("找到了PTActionConfig下的配置, 先删除新建所有子节点", XJ_LOG_LV3);
						//找到子节点列表指针
						MSXML2::IXMLDOMNodeListPtr pChild;
						pChild = pSelected ->GetchildNodes();
...
{
	WriteLogEx("CPTAction::FillListData, 开始填充列表");
	...
	WriteLogEx("CPTAction::FillListData, 填充列表正常结束");
	return 0;
}
...
BOOL CPTAction::LoadDataFromDB(int num, BOOL IsQueryAcked, int nQueryNo)
{
	...
	WriteLogEx("CPTAction::LoadDataFromDB, 开始查询");
	...
	{
		WriteLogEx("CPTAction::LoadDataFromDB, 查询失败");
		return FALSE;
	}
	if(nResult == 1)
	{
		...
		{
			CString str;
			str.Format("CPTAction::LoadDataFromDB, 读取到%d条数据", nCount);
			WriteLogEx(str);
		}
		...
		{
			CString str;
			str.Format("CPTAction::LoadDataFromDB, 要显示%d条数据,只找到%d条数据", m_nMaxRow, nChecked);
			WriteLog(str, XJ_LOG_LV3);
		}
	}
	else
	...
	WriteLogEx("CPTAction::LoadDataFromDB, 查询完毕,正常退出");
...
void CPTAction::OnSTTP20022( WPARAM wParam, LPARAM lParam )
{
	...
	{
		WriteLog("CPTAction::OnSTTP20022, 调试事件不显示", XJ_LOG_LV3);
		return;
	}
	...
	{
		WriteLog("CPTAction::OnSTTP20022, 返回事件不显示", XJ_LOG_LV3);
		return;
	}

	if(GetThreadCount() > 0)
	{
		WriteLog("CPTAction::OnSTTP20022, 正在载入历史数据,不处理报文", XJ_LOG_LV3);
		return;
	}
	...
	//加入到列表
	m_List.SetRedraw(FALSE);
	if (!AddEventToList(pEvent, 0)) //加入到第一行
	{
		WriteLog("CPTAction::OnSTTP20022 添加到列表失败", XJ_LOG_LV3);
	}
	//恢复刷新
	m_List.SetRedraw(TRUE);
	CheckLineCount();
...
/*************************************************************
 函 数 名：OnCustomDraw()
 功能概要：设置I,II,III类事件背景色, 未确认事件文字颜色
 返 回 值: void
 参    数：param1
		   Param2
**************************************************************/
...
int CALLBACK CPTAction::CompareFunction( LPARAM lParam1, LPARAM lParam2, LPARAM lParamData )
{
	...
	CString str1, str2;
	int iResult = 0;
	//"序号","类别", "故障类型", "装置启动时间", "信息接收时间", "消息内容", "已确认"
	switch(nCol)
	{
	case 0: //事件ID
...
{
	WriteLog("开始退出保护动作页面读取数据线程", XJ_LOG_LV3);
	...
	WriteLog("退出保护动作页面读取数据线程完毕", XJ_LOG_LV3);
}
...
CString CPTAction::TranslateSingleKey( CString sKey )
{
	...
	if(nFind > -1)
	{
		...
		{
			return pEvent->GetFaultType(); //故障类型
		}
...
void CPTAction::StartThread()
{
	...
	CString strLog;
	strLog.Format("保护动作查询线程启动成功,查询批次:%d", m_nQueryNo);
	WriteLog(strLog);
}
...
{
	CString str;
	str.Format("开始退出保护动作查询线程,查询号:%d", nNo);
	WriteLog(str, XJ_LOG_LV3);
	...
	str.Format("保护动作查询线程成功退出,查询号:%d", nNo);
	WriteLog(str, XJ_LOG_LV3);
}
...
void CPTAction::AddThreadNode( int nNo, HANDLE hThread )
{
	...
	CString str;
	str.Format("保护动作查询线程成功启动,查询号:%d", nNo);
	WriteLog(str, XJ_LOG_LV3);
	LeaveCriticalSection(&m_CriticalSection);
}
