...
//	vpo.y = 0;
	...
	if( HasEdit() && m_bSelected && IsEnabled() )
	{
		...
		m_pEdit->SetWindowText(m_pEdit->m_sString);
	//	m_pEdit->SetSel(0,-1);
		m_pEdit->SetSel(m_pEdit->m_sString.GetLength(), m_pEdit->m_sString.GetLength());
	}
	else
	{
		ASSERT(m_pEdit==NULL);
...
void EIconTextButtonProperty::OnPropActivate( EPropWnd* pParent )
{
	...
	if( HasEdit() && IsEnabled() )
	{
		...
		m_pEdit->m_sString = m_sInitialValueText;
